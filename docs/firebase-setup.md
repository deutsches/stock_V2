...
這筆資料代表當天凌晨已收盤的美股，加上當天 `14:30` 收盤的台股與可用現金。台幣現金直接計入，美元現金依目前匯率換算；總資產為「持股市值＋現金」。快照儲存在 `stockV2/users/{uid}/snapshots/{snapshotId}`。程式使用 Firebase transaction，只有當日文件不存在時才建立；使用者修改持股或現金時，則更新當日同一筆快照。既有的 `_0630` 歷史快照不會刪除；同一天若也有 `_1430`，圖表只採用 `_1430`，若舊日期只有 `_0630` 才沿用該筆。舊快照若沒有現金欄位，會以現金 0 相容顯示。
...
交易紀錄儲存在 `stockV2/users/{uid}/transactions/{transactionId}`，新紀錄保存年份、股票代號、名稱、已實現損益及報酬率；賣出價格為選填，沒有資料時顯示「—」。美股台幣損益暫依固定匯率 `1 USD = NT$30.33` 換算。讀取時仍相容早期含 `soldDate`，或以成本與收入計算損益的資料。
