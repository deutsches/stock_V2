...
test("快照分開保存原幣市場資料並計算台幣總額", () => {
  ...
  const snapshot = buildAssetSnapshot(holdings, 30.33, { date: "2026-08-17", slot: "1430" }, { twd: 1000, usd: 10 });

  assert.equal(snapshot.total.marketValueTwd, 2213.2);
  assert.equal(snapshot.cash.totalTwd, 1303.3);
  assert.equal(snapshot.total.totalAssetsTwd, 3516.5);
